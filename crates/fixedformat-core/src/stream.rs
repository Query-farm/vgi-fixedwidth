...
pub fn decompress_reader<R: BufRead + Send + 'static>(
    ...
    Ok(match codec {
        ...
        Compression::Gzip => Box::new(LabeledReader {
            inner: flate2::read::MultiGzDecoder::new(reader),
            label: "gzip decode",
        }),
        Compression::Zstd => Box::new(LabeledReader {
...
impl<R: BufRead> RecordStream<R> {
    ...
    /// `record_len` is required (non-zero) for [`Framing::Fixed`].
    pub fn new(mut reader: R, framing: Framing, record_len: usize) -> Result<Self> {
...
mod tests {
    ...
    /// Collect every record from a streaming framer over `data`.
    fn collect(data: &[u8], framing: Framing, record_len: usize) -> Result<Vec<Vec<u8>>> {
        RecordStream::new(Cursor::new(data.to_vec()), framing, record_len)?.collect()
    }
    ...
    fn fixed_zero_record_len_is_error() {
        assert!(RecordStream::new(Cursor::new(vec![1u8, 2, 3]), Framing::Fixed, 0).is_err());
    }
    ...
    fn forcing_gzip_on_plain_data_errors_with_gzip_label() {
        ...
        .unwrap();
        let err = RecordStream::new(std::io::BufReader::new(r), Framing::Newline, 0)
            .unwrap()
            .next()
            .unwrap()
            .unwrap_err();
    ...
    fn decompress_reader_autodetects_and_passes_through() {
        ...
        let mut out = Vec::new();
        decompress_reader(std::io::BufReader::new(Cursor::new(gz)), None)
            .unwrap()
            .read_to_end(&mut out)
            .unwrap();
