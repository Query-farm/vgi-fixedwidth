//! EBCDIC code page 037 ⇄ ASCII/Latin-1 byte transcoding.
//!
//! The tables are exact CP037 mappings generated from Python's built-in `cp037`
//! codec (EBCDIC byte → Unicode codepoint, taking codepoints < 256 as the
//! Latin-1 byte). Round-trip is lossless for all 256 byte values.

/// EBCDIC (CP037) byte → ASCII/Latin-1 byte.
pub static EBCDIC_TO_ASCII: [u8; 256] = [
    ...
    0xE8, 0xED, 0xEE, 0xEF, 0xEC, 0xDF, 0x21, 0x24, 0x2A, 0x29, 0x3B, 0xAC,
    ...
    0x79, 0x7A, 0xA1, 0xBF, 0xD0, 0xDD, 0xDE, 0xAE, 0x5E, 0xA3, 0xA5, 0xB7,
    0xA9, 0xA7, 0xB6, 0xBC, 0xBD, 0xBE, 0x5B, 0x5D, 0xAF, 0xA8, 0xB4, 0xD7,
...
/// ASCII/Latin-1 byte → EBCDIC (CP037) byte.
pub static ASCII_TO_EBCDIC: [u8; 256] = [
    ...
    0xE3, 0xE4, 0xE5, 0xE6, 0xE7, 0xE8, 0xE9, 0xBA, 0xE0, 0xBB, 0xB0, 0x6D,
    ...
    0x80, 0xFD, 0xFE, 0xFB, 0xFC, 0xAD, 0xAE, 0x59, 0x44, 0x45, 0x42, 0x46,
    0x43, 0x47, 0x9C, 0x48, 0x54, 0x51, 0x52, 0x53, 0x58, 0x55, 0x56, 0x57,
    0x8C, 0x49, 0xCD, 0xCE, 0xCB, 0xCF, 0xCC, 0xE1, 0x70, 0xDD, 0xDE, 0xDB,
    0xDC, 0x8D, 0x8E, 0xDF,
];

/// Transcode an EBCDIC byte to its ASCII/Latin-1 equivalent.
#[inline]
pub fn to_ascii(b: u8) -> u8 {
    EBCDIC_TO_ASCII[b as usize]
}

/// Transcode an ASCII/Latin-1 byte to its EBCDIC (CP037) equivalent.
#[inline]
pub fn to_ebcdic(b: u8) -> u8 {
    ASCII_TO_EBCDIC[b as usize]
}

/// Transcode a whole EBCDIC slice to a fresh ASCII/Latin-1 buffer.
pub fn decode_slice(src: &[u8]) -> Vec<u8> {
    src.iter().map(|&b| to_ascii(b)).collect()
}

/// Transcode a whole ASCII/Latin-1 slice to a fresh EBCDIC buffer.
pub fn encode_slice(src: &[u8]) -> Vec<u8> {
    src.iter().map(|&b| to_ebcdic(b)).collect()
}
...
mod tests {
    ...
    fn round_trips_all_bytes() {
        for b in 0u16..256 {
            let b = b as u8;
            assert_eq!(to_ebcdic(to_ascii(b)), b, "ebcdic round-trip failed at {b:#x}");
            assert_eq!(to_ascii(to_ebcdic(b)), b, "ascii round-trip failed at {b:#x}");
        }
    }

    #[test]
    fn known_letters_and_digits() {
        // CP037: 'A' = 0xC1, '0' = 0xF0, ' ' = 0x40.
        assert_eq!(to_ebcdic(b'A'), 0xC1);
        assert_eq!(to_ebcdic(b'0'), 0xF0);
        assert_eq!(to_ebcdic(b' '), 0x40);
        assert_eq!(to_ascii(0xC1), b'A');
        assert_eq!(to_ascii(0xF0), b'0');
        assert_eq!(to_ascii(0x40), b' ');
    }

    #[test]
    fn decodes_ebcdic_hello() {
        let ebcdic = [0xC8, 0xC5, 0xD3, 0xD3, 0xD6]; // "HELLO"
        assert_eq!(decode_slice(&ebcdic), b"HELLO");
        assert_eq!(encode_slice(b"HELLO"), ebcdic);
    }
}
