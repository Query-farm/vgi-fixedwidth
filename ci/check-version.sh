...
# version the worker advertises over VGI (`fixedformat_version()`) and the
# uploaded release binaries all equal the release. Run on a version-tag push
# before building/publishing anything.
#
...
# Read `version = "..."` from the [workspace.package] table. Restricting to that
# table avoids matching `rust-version` or a dependency's version line.
VERSION="$(awk '
